...
import com.gl.logcat.util.UtilLogger;
...
public class LogLine
{
	...
	if (logLevel == -1)
	{ // starter line like "begin of log etc. etc."
	    return logOutput;
	}
	...
	stringBuilder.append(convertLogLevelToChar(logLevel)).append('/').append(tag).append('(').append(processId).append("): ").append(logOutput);
	...
	// otherwise, it's a legacy log or the beginning of the log output or
	// something
	if (!TextUtils.isEmpty(originalLine) && Character.isDigit(originalLine.charAt(0)) && originalLine.length() >= TIMESTAMP_LENGTH)
	{
	...
	case 'W':
	    return Log.WARN;
	case 'F':
	    return LogLineAdapterUtil.LOG_WTF; // 'F' actually stands for 'WTF',
	    // which is a real Android log
	    // level in 2.2
	}
	return -1;
    }
